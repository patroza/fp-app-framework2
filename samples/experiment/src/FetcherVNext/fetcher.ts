...
export function bimap<S extends Status, E, A, G, B>(
  ...
  return (fetcher: Fetcher<S, E, A>) => ({
    input: fetcher.input,
    handlers: pipe(fetcher.handlers, R.map(RTE.bimap(f, g))) as Record<
      S,
      Decoder<G, B>
    >,
...
export function map<S extends Status, E, A, B>(
  f: (a: A) => B,
): (fetcher: Fetcher<S, E, A>) => Fetcher<S, E, B> {
  return bimap(identity, f)
}

export function mapLeft<S extends Status, E, A, G>(
  g: (e: E) => G,
): (fetcher: Fetcher<S, E, A>) => Fetcher<S, G, A> {
...
): ((fetcher: Fetcher<OldS, E, A>) => Fetcher<OldS | NewS, E, B>) =>
  flow(map(ab), extend(handlers))

export const handleError = (e: unknown) =>
  e instanceof Error ? e : new Error("unknown error")

export const stringDecoder: Decoder<Error, string> = response =>
  TE.tryCatch(() => response.text(), handleError)

export const jsonDecoder: Decoder<Error, unknown> = response =>
  TE.tryCatch(() => response.json(), handleError)

export function toTaskEither(
  fetch: Fetch,
): <S extends Status, E, A>(fetcher: Fetcher<S, E, A>) => TE.TaskEither<E, A> {
